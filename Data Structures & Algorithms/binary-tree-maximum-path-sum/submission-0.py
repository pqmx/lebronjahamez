...
class Solution:
    def maxPathSum(self, root: Optional[TreeNode]) -> int:
        return self.maxPathSums(root, 0);
    ...
    def maxPathSums(self, root: Optional[TreeNode], sum: int) -> int:
        if root is None:
            return 0
        if root.val < 0:
            return max(sum, self.maxPathSums(root.left, sum), self.maxPathSums(root.right, sum))
        
        # root has to pos
        sum += root.val
        left = self.maxPathSums(root.left, 0)
        right = self.maxPathSums(root.right, 0)
